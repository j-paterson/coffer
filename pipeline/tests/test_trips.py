...
def test_find_trips_clusters_travel_items(conn, seed_txn):
    seed_txn(
        ...
        postings=[("acct:chase", -800), ("equity:unknown-counterparty", 800)],
        item_category="Travel",
    )
    seed_txn(
        date="2026-03-12",
        description="MARRIOTT BANFF",
        postings=[("acct:chase", -400), ("equity:unknown-counterparty", 400)],
        item_category="Travel",
    )
    seed_txn(
        date="2026-04-15",
        description="UNITED AIRLINES",
        postings=[("acct:chase", -300), ("equity:unknown-counterparty", 300)],
        item_category="travel",
    )
