...
def test_find_transfer_pairs_basic_mirror(conn, seed_txn):
    a = seed_txn(
        ...
        postings=[("acct:chase", -500), ("equity:unknown-counterparty", 500)],
    )
    b = seed_txn(
        date="2026-04-02",
        description="to wealthfront",
        postings=[("acct:wealthfront", 500), ("equity:unknown-counterparty", -500)],
    )
...
def test_find_transfer_pairs_skips_non_transfer_categorized(conn, seed_txn):
    seed_txn(
        ...
        postings=[("acct:chase", -50), ("equity:unknown-counterparty", 50)],
...
def test_categorize_writes_to_synthesized_item(conn, seed_txn, tmp_path):
    tid = seed_txn(
        ...
        postings=[("acct:chase", -25), ("equity:unknown-counterparty", 25)],
    )
...
def test_categorize_only_uncategorized_skips_categorized_txns(
    ...
    seed_txn(
        ...
        postings=[("acct:chase", -10), ("equity:unknown-counterparty", 10)],
        item_category="shopping",
    )
    fresh = seed_txn(
        date="2026-04-02",
        description="UBER EATS fresh",
        postings=[("acct:chase", -12), ("equity:unknown-counterparty", 12)],
    )
    ruleset = load_rules(_write_rules(tmp_path, _UBER_RULES))
