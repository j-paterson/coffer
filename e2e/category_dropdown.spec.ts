...
// item_id=164, transaction_id=10013, account=Northwind Signature (7800).
...
test("setting a category via dropdown updates trigger text and persists across reload", async ({
  ...
  // Northwind Signature which holds the Google One single-item transaction.
  const cspSection = page.locator("section").filter({
    has: page.getByText("Northwind Signature"),
  });
  ...
  // Re-open Northwind Signature after reload (auto-opens Platinum Card).
