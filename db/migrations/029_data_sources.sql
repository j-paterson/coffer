...
--
-- A source can be both (e.g., an aggregator writes assertions AND snapshots).
...
INSERT INTO data_sources (name, kind, trust_rank, enabled, notes) VALUES
  ...
  ('bank-statement',   'assertion', 1, 1, 'Signed monthly statement PDF'),
  ('alt-k1',           'assertion', 2, 1, 'Tax K-1 ending capital'),
  ...
  ('aggregator',       'assertion', 6, 1, 'Live aggregator Financial.json snapshot'),
  ('aggregator-recap', 'assertion', 7, 1, 'Quarterly recap CSV (often underreports)'),
...
INSERT INTO data_sources (name, kind, trust_rank, enabled, notes) VALUES
  ...
  ('aggregator',               'snapshot', 3, 1, 'Aggregator snapshot'),
  ('zerion-chart',             'snapshot', 4, 1, 'Historical wallet chart'),
  ('backfill:txn-walk',        'snapshot', 5, 1, 'cost-basis importer qty × DefiLlama price'),
