...
--
-- Feeds into the TS-side FIFO cost-basis calculator alongside CoinTracker
-- events. To avoid double-counting trades that CoinTracker already saw,
-- the FIFO consumer skips derived events for symbols that CoinTracker
-- has any history of — the wallet is either entirely CoinTracker-tracked
-- (symbol shows up in CoinTracker history) or entirely Alchemy-derived
-- (symbol never appeared in CoinTracker).
