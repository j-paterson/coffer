...
export function Overview() {
  ...
  // Hovered point index for the chart's header readout (Net/Split modes).
    ...
    <div className="flex h-full min-h-0 max-w-5xl flex-col">
      ...
      <section className="mb-6 rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 border-t border-stone-100 pt-4">
          ...
          {!selectedAccount &&
            ...
            !selectedBundle &&
            (() => {
              // Unified header readout for all three modes — updates on scrub
              // via chartHoverIdx (reported by the active chart). Net shows
              // net worth; Split and Breakdown also show assets + debt.
              let date: string;
              let net: number;
              let assets: number | null = null;
              let debts: number | null = null;
              if (mode === "breakdown") {
                const snaps = privateBreakdownSnaps;
                if (!snaps || snaps.length === 0) return null;
                const hovering = chartHoverIdx != null && chartHoverIdx < snaps.length;
                const s = snaps[hovering ? chartHoverIdx! : snaps.length - 1];
                date = hovering ? s.as_of : "latest";
                net = s.total;
                assets = s.holdings.reduce((a, h) => a + (h.value_usd > 0 ? h.value_usd : 0), 0);
                debts = s.holdings.reduce((a, h) => a + (h.value_usd < 0 ? -h.value_usd : 0), 0);
              } else {
                if (!filteredPoints || filteredPoints.length === 0) return null;
                const hovering =
                  chartHoverIdx != null && chartHoverIdx < filteredPoints.length;
                const p = filteredPoints[hovering ? chartHoverIdx! : filteredPoints.length - 1];
                date = hovering ? p.date : "latest";
                net = p.net_worth;
                if (mode === "split") {
              ...
              return (
                <div className="mb-2 flex flex-wrap items-baseline gap-x-4 gap-y-0.5">
                  ...
                  <span className="flex items-baseline gap-1">
                    ...
                    <span className="text-xs text-stone-500">net worth</span>
                  </span>
                  {assets != null && debts != null && (
                    <>
                      <span className="flex items-baseline gap-1">
                        ...
                        <span className="text-xs text-stone-500">assets</span>
                      </span>
                      <span className="flex items-baseline gap-1">
                        <span className="font-mono text-sm tabular-nums text-rose-500">
                          {privacyOn ? "•••" : `-${fmt.amount(debts)}`}
                        </span>
                        <span className="text-xs text-stone-500">debt</span>
                      </span>
                    </>
                  )}
                </div>
          ...
          >
            {selectedBundle ? (
              bundleQ.isLoading || !filteredBundleSnaps ? (
                ...
                <ChartPlaceholder
                  height={260}
                  label="No history yet — it builds up as balances change over time."
                />
              ) : (
              ...
              <button
                ...
                onClick={() =>
                  setAccountActive.mutate({
                    ...
                    active: selectedAccount.active ? 0 : 1,
                  })
                }
                className="rounded-md border border-stone-200 px-2.5 py-1 text-xs font-medium text-stone-700 hover:bg-stone-50"
              >
                {selectedAccount.active ? "Archive" : "Unarchive"}
              </button>
              <button
                type="button"
                onClick={() => {
                  if (!confirm(`Delete "${selectedAccount.display_name_override ?? selectedAccount.display_name}"? This removes the account and its history.`)) return;
      ...
      {showAddAccount && (
        <AddAccountModal
          ...
          onSubmit={(data) => {
            createAccount.mutate(data, { onSuccess: () => setShowAddAccount(false) });
          }}
      ...
      {showBalanceModal && selectedAccount && (
        <AccountBalanceModal
          accountName={selectedAccount.display_name_override ?? selectedAccount.display_name}
          liability={isLiabilityType(selectedAccount.type)}
          pending={updateBalance.isPending}
          onSubmit={({ balance, as_of }) => {
            updateBalance.mutate(
              { accountId: selectedAccount.id, balance, as_of },
