...
export function useConnectProvider() {
  return useInvalidatingMutation(
    ({ id, fields }: { id: string; fields: Record<string, string> }) => api.connectProvider(id, fields),
...
export function useDisconnectProvider() {
  return useInvalidatingMutation(
    ({ id }: { id: string }) => api.disconnectProvider(id),
    [queryKeys.connections],
  );
}
...
export function useSetProviderEnabled() {
  return useInvalidatingMutation(
    ({ id, enabled }: { id: string; enabled: boolean }) => api.setProviderEnabled(id, enabled),
    [queryKeys.connections],
  );
}
...
export function useSyncProvider() {
  return useInvalidatingMutation(
    ({ id }: { id: string }) => api.syncProvider(id),
    [queryKeys.accounts, queryKeys.summary, queryKeys.connections],
  );
}
...
export function useUpdateAccountBalance() {
  return useInvalidatingMutation(
    ({ accountId, balance, as_of }: { accountId: string; balance: number; as_of: string }) =>
      api.updateAccountBalance(accountId, { balance, as_of }),
    [queryKeys.accounts, queryKeys.summary],
  );
}
...
export function useSetAccountActive() {
  return useInvalidatingMutation(
    ({ accountId, active }: { accountId: string; active: 0 | 1 }) =>
      api.setAccountActive(accountId, active),
    [queryKeys.accounts, queryKeys.summary],
  );
}
...
export function useDeleteAccount() {
  return useInvalidatingMutation(
    ({ accountId }: { accountId: string }) => api.deleteAccount(accountId),
    [queryKeys.accounts, queryKeys.summary],
  );
}
