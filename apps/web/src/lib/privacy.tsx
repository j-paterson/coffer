...
export function privacySnapshots(
  ...
  return snapshots.map((s, si) => {
    const holdings = s.holdings.map((h) => {
      ...
      // Preserve sign so debt buckets stay below the zero baseline even
      // when amounts are blurred.
      const sign = h.value_usd < 0 ? -1 : 1;
      return { symbol: h.symbol, value_usd: sign * base.value };
    });
