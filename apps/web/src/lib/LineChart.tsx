      ...
      {!drag && hoverX != null && (
        <>
          ...
          {(() => {
            ...
            const padX = 8;
            const w = 130;
            const h = headerH + series.length * rowH + 8;
            const x = xFor(hoverX);
            const left = Math.max(
              padding.left,
              Math.min(x - w / 2, padding.left + innerW - w),
            );
            const top = padding.top + 6;
            const dateStr = series[0]?.points[hoverX]?.x ?? "";
            return (
              <g transform={`translate(${left}, ${top})`} style={{ pointerEvents: "none" }}>
                ...
                {series.map((s, i) => {
                  const v = s.points[hoverX]?.y ?? 0;
                  return (
                    <g
                      key={s.key}
                      transform={`translate(${padX}, ${headerH + 4 + i * rowH})`}
                    >
                      <text x={0} y={9} className="fill-white text-[9px]">
                        {s.label}
                      </text>
                      <text
                        x={w - padX * 2}
                        y={9}
                        textAnchor="end"
                        className="fill-white text-[9px] font-medium tabular-nums"
                      >
                        {formatY(v)}
                      </text>
                    </g>
