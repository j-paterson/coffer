...
route.get("/breakdown", (c) => {
  ...
  const { seriesByAcct, globalStart, globalEnd, accounts } = w;
  ...
  while (cursor <= globalEnd) {
    ...
    for (const [canonical, series] of seriesByAcct) {
      const v = series.get(cursor);
      // Include both signs: assets (v > 0) stack above the zero baseline,
      // debts (v < 0 — credit cards, a negative-balance mortgage) mirror
      // below it. `total` is therefore net worth (assets - debts), matching
      // the Net series' right edge.
