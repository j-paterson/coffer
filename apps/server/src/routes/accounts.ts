...
const ACCOUNT_CATEGORY_IDS = new Set(ACCOUNT_CATEGORIES.map((c) => c.id));
...
route.patch("/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json<{ display_name_override?: string | null; active?: number }>();
  const value =
  ...
  if (typeof body.active === "number") {
    const acct = ctx.db
      .prepare("SELECT mode FROM accounts WHERE id = ?")
      .get(id) as { mode: string } | undefined;
    if (!acct) return c.json({ error: "account not found" }, 404);
    if (acct.mode !== "manual")
      return c.json({ error: "provider accounts cannot be archived here" }, 409);
    ctx.db
      .prepare("UPDATE accounts SET active = ? WHERE id = ?")
      .run(body.active ? 1 : 0, id);
    return c.json({ id, active: body.active ? 1 : 0 });
  }
  const result = ctx.db
...
async function* toOps(ops: Operation[]): AsyncIterable<Operation> {
...
/** Generate a unique `manual:<slug>` id. */
function manualId(db: import("bun:sqlite").Database, name: string): string {
  const base = `manual:${slugify(name)}`;
  let id = base;
  let n = 2;
  while (db.prepare("SELECT 1 FROM accounts WHERE id = ?").get(id)) {
    id = `${base}-${n++}`;
  }
  return id;
}
...
route.post("/", async (c) => {
  ...
  const category = body.category as AccountCategory;
  const balance = Number(body.balance);
  const as_of = body.as_of ?? ctx.today;

  if (!name) return c.json({ error: "display_name is required" }, 400);
  if (!ACCOUNT_CATEGORY_IDS.has(category)) return c.json({ error: "invalid category" }, 400);
  if (!Number.isFinite(balance)) return c.json({ error: "balance must be a number" }, 400);
  if (!ISO_DATE.test(as_of)) return c.json({ error: "as_of must be YYYY-MM-DD" }, 400);

  const meta = categoryMeta(category);
  const id = manualId(ctx.db, name);
  await runOperations(
    ctx.db,
    toOps([
      {
        ...
        draft: {
          id,
          display_name: name,
          institution: (body.institution ?? "").trim() || "Manual",
          type: meta.type,
      ...
      },
      {
        ...
        draft: {
          account_id: id,
          as_of,
          expected_usd: signedBalance(category, balance),
          source: "manual",
  ...
  const acct = ctx.db
    .prepare("SELECT id, display_name, institution, type, currency, active, mode FROM accounts WHERE id = ?")
    .get(id);
  return c.json(acct);
});

route.post("/:id/balance", async (c) => {
  const ctx = c.get("ctx") as Ctx;
  const id = c.req.param("id");
  const body = await c.req.json<{ balance?: unknown; as_of?: string }>();
  const balance = Number(body.balance);
  const as_of = body.as_of ?? ctx.today;

  const acct = ctx.db
    .prepare("SELECT type, mode FROM accounts WHERE id = ?")
    .get(id) as { type: string; mode: string } | undefined;
  if (!acct) return c.json({ error: "account not found" }, 404);
  if (acct.mode !== "manual")
    return c.json({ error: "provider accounts are managed by their sync" }, 409);
  if (!Number.isFinite(balance)) return c.json({ error: "balance must be a number" }, 400);
  if (!ISO_DATE.test(as_of)) return c.json({ error: "as_of must be YYYY-MM-DD" }, 400);

  const signed = isLiabilityType(acct.type) ? -Math.abs(balance) : Math.abs(balance);
  await runOperations(
    ctx.db,
    toOps([
      {
        kind: "assertion",
        draft: { account_id: id, as_of, expected_usd: signed, source: "manual", source_file: "manual-entry" },
      },
    ]),
  );
  return c.json({ id, as_of, expected_usd: signed });
});

route.delete("/:id", (c) => {
  const ctx = c.get("ctx") as Ctx;
  const id = c.req.param("id");
  const acct = ctx.db.prepare("SELECT mode FROM accounts WHERE id = ?").get(id) as
    | { mode: string }
    | undefined;
  if (!acct) return c.json({ error: "account not found" }, 404);
  if (acct.mode !== "manual")
    return c.json({ error: "provider accounts cannot be deleted here" }, 409);
  ctx.db.prepare("DELETE FROM reconciliation_notes WHERE account_id = ?").run(id);
  ctx.db.prepare("DELETE FROM balance_assertions WHERE account_id = ?").run(id);
